...
- 类别概率:
{newline.join([f'  - {cat}: {prob*100:.1f}%' for cat, prob in classification['probabilities']])}

## 相似文本
{newline.join([f'{i+1}. {text["text"]}{newline}   相似度: {text["similarity"]*100:.1f}%' for i, text in enumerate(similar_texts)])}
